...
export default function ParentFilterModal({
  classes = [],
  grades = [],
}: {
  classes: ClassOption[];
  ...
  const activeFilterCount = [selectedClass, selectedGrade, selectedStatus].filter(Boolean).length;

  const applyFilters = (cls: string, grd: string, st: string) => {
  ...
  const handleApply = () => {
    applyFilters(selectedClass, selectedGrade, selectedStatus);
    setIsOpen(false);
  };
          ...
          <div className="absolute right-0 mt-2 w-80 bg-white rounded-2xl shadow-xl border border-gray-100 p-5 z-50 flex flex-col gap-4 animate-in fade-in zoom-in-95 duration-150">
            <div className="flex items-center justify-between border-b border-gray-100 pb-3">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-bold text-gray-800 text-sm">Filter Parents</h3>
              </div>
              <button
                onClick={() => setIsOpen(false)}
            ...
            {/* Filter 1: Child's Class / Section */}
            ...
            <div className="flex flex-col gap-1.5">
              ...
              >
                ...
                <option value="linked">Has Linked Children</option>
